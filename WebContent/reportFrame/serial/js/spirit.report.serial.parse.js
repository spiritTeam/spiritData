...
var d_tagDeal = {
  ...
  drawTagArea:function(jqEle, dtag, d) {
    if (dtag.showType=="value") d_tagDeal.draw.drawValue(jqEle, dtag, d);
      ...
      var errMsg = d_tagDeal.tableDataFun.checkMatch(dtag, d); //检查数据的匹配情况，并把结果追加到dtag中的tdParseData中去
  ...
  draw:{
    ...
    drawTable:function(jqEle, tdata) {
      var fTdata = d_tagDeal.tableDataFun.filterData(dtag, d);
      if (fTdata.errMsg) htmlStr = "<span class='errMsgSpan'>"+errMsg+"</span>";
      else {
        //获得列对象
        ...
        var gridWidth=0;
        cols[0]=new Array();
        for (var i=0; i<fTdata.titles.length; i++) {
          oneT = fTdata.titles[i];
          for(var p in oneT) {
  ...
  tableDataFun:{
    ...
    filterData:function(dtag, d) {
      ...
      if (isAllTitels) {
        ...
        ret.titles = new Array();
        for (i=0; i<mergeTitle.length; i++) {
          found=false;
          for (j=0; j<d.titles.length; j++) {
            for (var p in d.titles[j]) {
              if (p==mergeTitle[i]) {
                find=true;
                break;
              }
          ...
          if (find) ret.titles.push(d.titles[j]);
        }
      }
      if (tp.percentL&&tp.percentL.length>0) {//处理百分比列
          ...
          if (find) {
          	var _a="perc(00)", _b="中文";
          	var _c = eval("({'"+_a+"':'"+_b+"'})");
          	var oneT = eval("({'percent("+tp.percentL[i]+")':'"+titleName+"占比'})");
          	
          	ret.titles.push(d.titles[j]);
          }
      	}
      }
    ...
    checkMatch:function(dtag, d) {
      ...
      if (!_defineTitles||_defineTitles.length==0) return "表数据不合法，titles没有定义。";
      //匹配参数列
      ...
      //匹配排序列
      if (tp.sort&&tp.sort.sortCol) {
        var find=false, _b, _a=tp.sort.sortCol;
        for (var j=0; j<_defineTitles.length; j++) {
          _b = _defineTitles[j];
          for (var p in _b) {
            if (p==_a) {
              find=true;
